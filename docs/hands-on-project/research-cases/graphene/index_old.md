...
![Graphene Structure](../../../assets/images/research-cases/graphene.jpg)
...
Before running the calculation, load the Quantum ESPRESSO environment:
...
The Self-Consistent Field (SCF) calculation is the first step

to obtain the converged electronic density.


Submit the SCF calculation using Slurm:
...
After SCF convergence, the NSCF calculation is performed

to obtain electronic states using a denser k-point grid.


Submit the NSCF calculation:
...
```bash
grep "JOB DONE" output/graphene_nscf.out
```
...
The DOS calculation uses the electronic states generated

from the NSCF calculation.
...
After successful calculations, the following files are generated:


| File | Description |
|---|---|
| graphene_scf.out | SCF calculation output |
| graphene_nscf.out | NSCF calculation output |
| graphene_dos.dat | Density of States data |
| graphene_bands.dat.gnu | Band structure data |
...
The following Slurm scripts are provided to run
Quantum ESPRESSO calculations on HPC.


### QE Submission Script


Submit SCF, NSCF, and band structure calculations:
...
Run DOS and band post-processing:
...
A workflow helper script is provided to guide the
...
[Download graphene_analysis.ipynb](analysis/graphene_analysis.ipynb)
...
The electronic structure results can be analyzed using
